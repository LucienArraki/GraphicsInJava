...
public class Zad1c1 {
    ...
    public Zad1c1(int x,int y, int width,int backColour,int fronColour)
    {
        ...
        for ( i = 0; i < y_res; i++)
            for ( j = 0; j < x_res; j++)
            {
                boolean right = (i/w) % 2 != 0 && (j/w) % 2 != 0;
                boolean left = (i/w) % 2 == 0 && (j/w) % 2 == 0;
                if (right || left)
        ...
        try
        {
            ImageIO.write( image, "bmp", new File( "1c1.bmp" ) );
